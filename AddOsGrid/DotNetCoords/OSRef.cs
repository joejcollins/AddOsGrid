...
namespace DotNetCoords
{
  ...
  public class OSRef : CoordinateSystem 
  {
    ...
    public OSRef(string gridRef) : base(OSGB36Datum.Instance)
    {
      ...
      switch (gridRef.Length)
      {
        ...
        case 12: 
          ...
          length = 5;
          break;
        default: throw new ArgumentException("Unexpected length of grid reference (should be 8, 10 or 12 characters long)", "gridRef");
      }

      char char1 = gridRef[0];
      char char2 = gridRef[1];
      // Thanks to Nick Holloway for pointing out the radix bug here
      int east = int.Parse(gridRef.Substring(2, length)) * multiplier;
      int north = int.Parse(gridRef.Substring(2+length, length)) * multiplier;
    ...
    /// six-figure notation in the form XY 12345 67890.
    /// </summary>
    /// <returns>A string representing this OSGB grid reference in ten-figure
    /// notation</returns>
    public string ToTenFigureString()
    {
      int e = (int)Math.Floor((easting - (100000 * HundredKmEast())));
      int n = (int)Math.Floor((northing - (100000 * HundredKmNorth())));

      string es = e.ToString("D5");
      string ns = n.ToString("D5");
    ...
    {
      int e = (int)Math.Floor((easting - (100000 * HundredKmEast())) / 100);
      int n = (int)Math.Floor((northing - (100000 * HundredKmNorth())) / 100);
      
      string es = e.ToString("D3");
      string ns = n.ToString("D3");
    ...
    {
      return (int)Math.Floor(easting / 100000);
    }

    private int HundredKmNorth()
    {
      return (int)Math.Floor(northing / 100000);
    }
    ...
    private string GetGridReferenceLetters()
    {
      ...
      int index = 65 + ((4 - (HundredKmNorth() % 5)) * 5) + (HundredKmEast() % 5);
